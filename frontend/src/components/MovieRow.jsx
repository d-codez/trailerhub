...
const MovieRow = ({ title, items, onPlayTrailer, onMoreInfo, mediaType = 'movie' }) => {
  ...
  return (
    <div className='relative mb-8 md:mb-12 group/row'>
      <h2 className='text-white text-lg md:text-xl font-semibold mb-2 md:mb-4 px-4 md:px-12'>{title}</h2>
      ...
      <div className='relative'>
        ...
        <div
          ...
          className='flex gap-2 md:gap-3 overflow-x-scroll scrollbar-hide px-4 md:px-12 pb-40 -mb-36'
          style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          {items.map((item) => (
            <MovieCard key={item.id} item={item} onPlayTrailer={onPlayTrailer} onMoreInfo={onMoreInfo} mediaType={mediaType} />
          ))}
        </div>
